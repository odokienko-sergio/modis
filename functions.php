...
function modis_scripts() {
	...
	wp_enqueue_script('filter', get_template_directory_uri()."/assets/js/filter.js", array(), _S_VERSION, true );
...
function modis_init_widgets() {
	register_widget('Modis_Filter_Widget');
}
add_action('widgets_init', 'modis_init_widgets');
...
function modis_show_products() {
	...
	$orderby = isset($query_data['order']) ? $query_data['order'] : "date";

	$posts_per_page = get_option('woocommerce_catalog_columns') * get_option('woocommerce_catalog_rows');

	// Filter by category id
	$cats = (isset($query_data['category']) && !empty($query_data['category'])) ? $query_data['category'] : false;
    ...
    <?php
		...
		add_action('wp_ajax_modis_filter', 'modis_show_products');
		add_action('wp_ajax_nopriv_modis_filter', 'modis_show_products');
